...
> [!IMPORTANT]
> **SSOT는 `bess-output-generator.md`다.** 아래는 요약이며, 여백·폰트·표 폭 등 세부 수치는
> `bess-output-generator.md`의 "여백 가이드 — 3가지 옵션"·"타이포그래피 기준" 절이 정본이다.
> 이 문서(CLAUDE.md)가 예전에 별도 고정값(11pt/10.5pt·좌2.5·우2.0cm 단일 여백)을 들고 있어
> `bess-output-generator.md`(본문 12pt 통일·여백 옵션 A/B/C)와 어긋나 있던 것을 2026-07-21
> 발견·정정했다(마켓 대시보드 보고서 좌우 여백 불균형 민원이 근본 원인이었음). 두 문서가
> 다시 어긋나면 **`bess-output-generator.md` 수치를 우선**한다.
...
여백 (bess-output-generator.md 3옵션 결정트리 따름 — 상세는 해당 문서 참조):
├─ 옵션 A(대칭 25/25/25/25mm): 디지털 배포·화면 검토·PDF 첨부 ← 기본값
├─ 옵션 B(좌30/우20mm): 단면 인쇄 + 제본 예정(스테이플·바인더)
└─ 옵션 C(mirror 25/25/inside30/outside20mm): 양면 인쇄 책자형(50p+)

헤더/푸터 정보 (프로젝트 산출물 — 계약서/절차서/기술사양서 등 정식 문서용):
...
└─ 우하: [QR코드] (파일 위치/링크)
  ※ 특정 프로젝트에 속하지 않는 정기 산출물(예: 마켓 인텔리전스 보고서)은 프로젝트 코드·
    서명란·QR코드가 의미 없으므로 생략 가능 — bess-output-generator.md의 A4/160mm표/TOC/
    페이지번호 기준만 적용.

폰트 (bess-output-generator.md와 동일, 전 출력물 공통):
├─ 본문: 12pt 통일 (영문 Calibri / 한글 맑은고딕) ← ★ 표 본문도 동일
├─ 제목(H1): 16pt Bold Navy / 소제목(H2): 13pt Bold Blue
└─ 단위/기호: Times New Roman 10pt (해당 시)
