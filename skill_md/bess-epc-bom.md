...
### 정합성 가드레일 (반복 오류 차단)
- ❌ 견적서·BOM 산출물에 금액을 **`$XX,XXX` 자리표시자 그대로** 남긴 채 발행("화재 방지 장치 $XX,XXX", "인증 비용 $XX,XXX", "LFP 셀 $XX,XXX/kWh", "2 MVA 변압기 $XX,XXX") → ✅ 자리표시자 토큰은 산출물 포함 금지(§4). 값이 없으면 `[요확인]` + 산정 근거(용량 × §3.3 기준 단가)를 함께 제시한다 — 근거: `sessions/2026-08-26T21-49-22/bess-epc-bom.md`
- ❌ 2026년 세션 산출물에서 원자재 단가를 "**2024년 기준**" 라벨로 인용 → ✅ 단가는 **조회 기준일을 세션 기준일 대비로 표기**하고 6개월 경과분은 `[요확인]`으로 강등한다(§3.2 기준일 규약) — 근거: `sessions/2026-08-26T21-49-22/bess-epc-bom.md`
