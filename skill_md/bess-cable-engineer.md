...
- 시장별 허용전류표·설치 기준 소관(세션 정리): **KR = KEPCO 허용전류표 · KEPCO ES-6120** / **US = NEC 310.16 · IEEE 835 Ampacity 표**. 매설 깊이는 **IEC 60364-5-52**(직매 0.8~1.0 m), 난연 등급은 **LSZH**, 디레이팅은 k₁(주변온도)·k₂(토양 열저항)로 분리 적용 — 근거: `sessions/2026-08-28T15-44-12/bess-cable-engineer.md`
### 정합성 가드레일 (반복 오류 차단)
- ❌ 22.9 kV·10 MVA 회로의 부하전류를 **I_load = 8,000 A**로 두고 이용률 **U = 8,000/500 = 16**을 산출한 뒤 "1 이하로 유지 필요"라는 문장만 덧붙여 발행 → ✅ §3.4 검산 기준값은 **10 MW @ 22.9 kV = 252 A**다. U > 1이면 결과를 발행하지 않고 재계산한다(§5-2 자릿수 검산·§5-5 비율식 라벨) — 근거: `sessions/2026-08-28T15-44-12/bess-cable-engineer.md`
- ❌ 설계 여유를 "**I_ampacity ≥ 1.25 × I_load = 10,000 A(10 kA)**"로 제시 → ✅ 케이블 허용전류의 통상 범위는 **수백 A**다. 통상 범위를 10배 이상 벗어나면 발행하지 않고 재계산한다(§5-2) — 근거: `sessions/2026-08-28T15-44-12/bess-cable-engineer.md`
- ❌ 최소 굴곡반경을 "**도체 직경의 10배 → 240 mm² 케이블은 2.4 m**"로 산정 → ✅ **240 mm²는 단면적**이고 굴곡반경 기준은 **케이블 외경(OD)의 10~15배**다. 단면적 수치를 mm 직경으로 읽어 10배를 곱하지 않는다 — 근거: `sessions/2026-08-28T15-44-12/bess-cable-engineer.md`
- ❌ 같은 문서에서 MV 전압강하 한계를 "**≤3%**"로 쓰고 예시값을 "**≤73.4 V**"로 병기 → ✅ 22.9 kV의 3%는 **687 V**다. 한계는 %로 정의하고 V 환산값을 함께 쓸 때 두 값이 일치하는지 검산한다(§5-7 문서 내 값 단일화) — 근거: `sessions/2026-08-28T15-44-12/bess-cable-engineer.md`
